...
using a Gaussian policy is almost a contradiction, if
the policy is supposed to be deterministic.
'''
import sys
...
from DDPG_NN_con_gauss import NN_PI_s
from DDPG_NN_con_gauss import NN_Q_s_a
...
class DeepDeterministicPolicyGradient:
    ...
    def __init__(self,
        ...
        with tf.variable_scope('Actor_con'):
            mu, sigma   = NN_PI_s(self.s,  n_actions, action_bound,  'eval_net', trainable=True) #[None, 1]
            mu_, sigma_ = NN_PI_s(self.s_, n_actions, action_bound, 'target_net', trainable=False) #[None, 1]

            self.mu = action_bound * mu #[None, 1]
            self.sigma = sigma #[None, 1]
            self.normal = tf.distributions.Normal(self.mu, self.sigma)
            self.tmp1 = self.normal.sample(1) #[1, None, 1]
            self.tmp2 = tf.squeeze(self.tmp1, axis=0) #[None, 1]
            self.a = tf.clip_by_value(self.tmp2, -action_bound, action_bound) #[None, 1]

            self.mu_ = action_bound * mu_ #[None, 1]
            self.sigma_ = sigma_ #[None, 1]
            self.normal_ = tf.distributions.Normal(self.mu_, self.sigma_)
            self.a_ = tf.clip_by_value(tf.squeeze(self.normal_.sample(1), axis=0), -action_bound, action_bound)
    ...
    def learn(self):   # batch update
        ...
        #tmp1, tmp2, a = self.sess.run([self.tmp1, self.tmp2, self.a], feed_dict={self.s: s})
