...
class DeepQNetwork:
    ...
    def __init__(
        ...
        self.error = (self.n_actions * tf.reduce_mean(self.tmp)) #(1, ) (n_actions-1) are zeros in each sub-array for this mean.
        #self.train_op = tf.train.RMSPropOptimizer(self.lr).minimize(self.error)
        #the benefit if we do this is the option to acumulate the gradients on a
        #temporal NN and apply_gradients after n-steps:
        self.OptC      = tf.train.RMSPropOptimizer(self.lr)
        self.c_grads   = tf.gradients(self.error, e_params)
        self.train_op = self.OptC.apply_gradients(zip(self.c_grads, e_params))

        self.replace_target_op = [tf.assign(t, e) for t, e in zip(t_params, e_params)]
        #self.replace_target_op = tf.group(*[tf.assign(t, e) for t, e in zip(t_params, e_params)])
