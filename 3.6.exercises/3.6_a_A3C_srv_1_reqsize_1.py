...
class runDataCenter_A3C:
    ...
    def run(self):
        ...
        self.workers[0].AC.plot_graph(self.gE.r_episode_series.all_elements(), supercase + "__episodes__r_", do_not_save=True)
        self.workers[0].AC.plot_graph(self.gE.r_episode_series.cumulative_avg_all(), supercase + "__avg_all__r__", do_not_save=True)
        self.workers[0].AC.plot_graph(self.workers[0].error_history, "error", True)
        self.workers[0].AC.plot_a_2D_DataCenter("last_episode", do_not_save=True)
        self.workers[0].AC.plot_V_a_1D_DataCenter("last_episode", do_not_save=True)
        ...
        for episode in range(int(self.rDC.max_num_episodes/self.rDC.N_WORKERS)):
            ...
            while True:
                ...
                if steps_in_episode % 1000 == 0:
                    print(' Worker: ', self.name,
                          '\t| r_in_episode: ', round(r_sum_in_episode*suma_rwd / steps_in_episode, 4),
                          )
                '''
                ...
                if done:
                    ...
                    globalEpi, r_avg = self.gE.done(r_sum_in_episode*suma_rwd,
                                                    steps_in_episode,
                                                    acts_in_episode)
