    ...
    def run(self):
        ...
        for episode in range(self.max_num_episodes):
            ...
            while True:
                ...
                success = env.action_map_req_on_less_loaded(a)
                while success:
                    s_iter = env.observation()
                    a_iter = self.epsilon_greed_policy(epsilon, s_iter)
                    success = env.action_map_req_on_less_loaded(a_iter)
                # s_ is not all right, as it is the result of several actions,
                # but it works, all other parameters are right.
                s_, r, done, new_req = env.step_ite()
